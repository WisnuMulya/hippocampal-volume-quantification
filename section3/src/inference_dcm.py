...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    for idx, slice_n in enumerate(slice_nums):
        slice = orig_vol[:, :, slice_n]
        mask = pred_vol[:, :, slice_n]

        # Convert slice and mask to PIL images
        nd_img = np.flip((slice / np.max(slice)) * 0xFF).T.astype(np.uint8)
        nd_ant = np.flip(mask == 1).T.astype(np.uint8)
        nd_post = np.flip(mask == 2).T.astype(np.uint8)
        nd_ant = np.stack(
            (nd_ant * 255, nd_ant * 0, nd_ant * 0, nd_ant * 64), axis=-1
        )  # Red with 25% opacity
        nd_post = np.stack(
            (nd_post * 255, nd_post * 0, nd_post * 0, nd_post * 128), axis=-1
        )  # Red with 50% opacity

        # Create PIL images
        pil_i = (
            Image.fromarray(nd_img, mode="L")
            .convert("RGBA")
            .resize((slice.shape[0] * 8, slice.shape[1] * 8))
        )
        pil_ant = Image.fromarray(nd_ant).resize(
            (slice.shape[0] * 8, slice.shape[1] * 8)
        )
        pil_post = Image.fromarray(nd_post).resize(
            (slice.shape[0] * 8, slice.shape[1] * 8)
        )

        # Paste the PIL images into the main report image object
        pil_i.alpha_composite(pil_ant)
        pil_i.alpha_composite(pil_post)
        pimg.paste(pil_i, ((idx * 320) + 10, 280))
...
def get_series_for_inference(path):
    ...
    series_for_inference = [
        dcm for dcm in dicoms if "HippoCrop" in dcm.SeriesDescription
    ]
...
if __name__ == "__main__":
    ...
    inference_agent = UNetInferenceAgent(
        device="cpu",
        parameter_file_path=r"../../section2/out/2024-11-29_0428_Basic_unet/model.pth",
    )
    ...
    os_command("sudo bash ./deploy_scripts/send_result.sh")
